...
import software.amazon.awssdk.services.s3.model.{CreateBucketRequest, GetObjectRequest, HeadObjectRequest, HeadObjectResponse, NoSuchKeyException}
...
class MyLocalStackContainer private(
  ...
  import MyLocalStackContainer._
...
class SimpleS3UploaderTest extends AnyFlatSpec with Matchers with TestContainerForEach {
  ...
  "SimpleS3Uploader" should "upload a file in the desired bucket" in {withContainers { ls =>
    ...
    val uploader = new SimpleS3Uploader(
      region = region,
    ...
    uploader.upload(BucketName, Paths.get("build.sbt"))
    ...
    val response: HeadObjectResponse = s3.headObject(request)
    ...
    val request1 = GetObjectRequest
      .builder()
      .key("build.sbt")
      .bucket(BucketName)
      .build()

    s3.getObjectAsBytes(request1)
    ...
    Try(
      ...
      bytes => {
        val actualResult = new String(bytes.asByteArray())

        assert(actualResult.nonEmpty)
      }
    )
  }
  }
